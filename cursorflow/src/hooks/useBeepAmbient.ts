...
interface BeepAmbientOptions {
  enabled?: boolean;
  baseVolume?: number;
}

export const useBeepAmbient = ({
  enabled = false,
  baseVolume = 0.5
}: BeepAmbientOptions = {}) => {
  ...
  const initAudio = useCallback(async () => {
    ...
    try {
      ...
      if (audioContextRef.current.state === 'suspended') {
        try {
          await audioContextRef.current.resume();
          console.log('Beep audio context resumed successfully');
        } catch (error) {
          console.error('Failed to resume beep audio context:', error);
      ...
      console.error('Failed to initialize beep system:', error);
    }
  }, [enabled, baseVolume]);

  // Update beep from mouse movement
  const updateBeepFromMouse = useCallback((x: number, y: number, velocity: number) => {
  ...
  const stopBeep = useCallback(() => {
    ...
    try {
      ...
      console.error('Error stopping beep system:', error);
    }
  }, []);
